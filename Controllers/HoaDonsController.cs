...
namespace NemeShop.Controllers
{
    ...
    public class HoaDonsController : Controller
    {
        ...
        public HoaDonsController(QuanLyTapHoaContext context, IHubContext<RealtimeHub> hub)
        {
            _context = context;
            _hub = hub;
        }
        ...
        public IActionResult Create()
        {
            ...
            ViewBag.MaVoucher = new SelectList(_context.MaGiamGia.Where(v => v.TrangThai), "MaVoucher", "MaVoucher");
            return View();
        }
        ...
        public async Task<IActionResult> CreateAjax([FromBody] HoaDonDto dto)
        {
            ...
            {
                var voucher = await _context.MaGiamGia.FirstOrDefaultAsync(v => v.MaVoucher == dto.MaVoucher && v.TrangThai);
                if (voucher != null && DateTime.Now >= voucher.NgayBatDau && DateTime.Now <= voucher.NgayKetThuc && voucher.SoLuong > voucher.DaSuDung && tienGoc >= (voucher.GiaTriDonToiThieu ?? 0))
                {
                    if (voucher.PhanTramGiam.HasValue) tienGiam = tienGoc * voucher.PhanTramGiam.Value / 100;
                    if (voucher.GiamTienTrucTiep.HasValue) tienGiam += voucher.GiamTienTrucTiep.Value;
            ...
            // notify
            await _hub.Clients.All.SendAsync("OrderStatusChanged", hoaDon.MaHd, hoaDon.TrangThai);

            return Json(new { success = true, message = "Thêm hóa đơn thành công!", orderId = hoaDon.MaHd });
        }
        ...
        {
            var voucher = _context.MaGiamGia.FirstOrDefault(v => v.MaVoucher == maVoucher && v.TrangThai);
            if (voucher == null) return Json(new { success = false, message = "Voucher không hợp lệ" });

            decimal tienGiam = 0;
            if (voucher.PhanTramGiam.HasValue) tienGiam += tongTien * voucher.PhanTramGiam.Value / 100;
            if (voucher.GiamTienTrucTiep.HasValue) tienGiam += voucher.GiamTienTrucTiep.Value;
        ...
        private async Task<decimal> TinhTienGiam(string? maVoucher, decimal tongTien)
        {
            ...
            if (DateTime.Now < voucher.NgayBatDau || DateTime.Now > voucher.NgayKetThuc) return 0;
            if (tongTien < (voucher.GiaTriDonToiThieu ?? 0)) return 0;
